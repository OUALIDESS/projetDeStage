...
const DAECPage = ({ theme }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const daec = divisionResponse.data.find((div) => div.name === 'DAEC');
      if (!daec) {
      ...
      if (filteredEmployees.length === 0) {
        setError('No employees found for DAEC');
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DAEC Employees');
      XLSX.writeFile(wb, 'daec_employees.xlsx');
    } catch (err) {
