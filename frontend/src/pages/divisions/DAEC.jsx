...
const DAEC = () => {
  ...
  useEffect(() => {
    const fetchDAECEmployees = async () => {
      try {
        ...
        const daec = divisionRes.data.find(d => d.name === 'DAEC');
        if (!daec) {
        ...
        const employeeRes = await axios.get('/api/employees', {
          headers: { Authorization: `Bearer ${token}` },
          params: { ids: daec.employeeIds.join(',') }
        });
        setEmployees(employeeRes.data);
      } catch (err) {
        setError('Failed to fetch employees');
        console.error('Error fetching DAEC employees:', err);
      }
    };
    fetchDAECEmployees();
  }, [navigate]);
  ...
  const handleSaveEmployee = async () => {
    try {
      ...
      if (editMode) {
        ...
        const daec = divisionRes.data.find(d => d.name === 'DAEC');
        if (daec) {
          await axios.put(`/api/divisions/${daec._id}`, {
            ...daec,
            employeeIds: [...daec.employeeIds, response.data._id]
          }, {
            headers: { Authorization: `Bearer ${token}` },
          });
        }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (daec) {
        await axios.put(`/api/divisions/${daec._id}`, {
          ...daec,
          employeeIds: daec.employeeIds.filter(id => id !== employeeToDelete._id)
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Employés DAEC');
    XLSX.writeFile(wb, 'employes_daec.xlsx');
  };
  ...
  return (
    <div className="bg-white rounded-3 shadow-sm p-4 mx-auto" style={{ maxWidth: '1200px', margin: '20px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="mb-0 fw-semibold">Gestion des missions - DAEC</h5>
