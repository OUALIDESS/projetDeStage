...
const DivisionPage = ({ theme }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const division = divisionResponse.data.find((div) => div.name === divisionName);
      if (!division) {
      ...
      const filteredEmployees = employeeResponse.data.filter((emp) => {
        ...
        return divisionId === division._id || divisionId?.toString() === division._id.toString();
      });
      setEmployees(filteredEmployees);

      if (filteredEmployees.length === 0) {
        setError(`No employees found for ${divisionName}`);
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, `${divisionName} Employees`);
      XLSX.writeFile(wb, `${divisionName.toLowerCase()}_employees.xlsx`);
    } catch (err) {
      ...
      <div className="container-lg">
        <div className="bg-white rounded-3 p-4 shadow-sm" style={{ backgroundColor: 'var(--card-bg)' }}>
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h2 className="mb-0" style={{ color: 'var(--text-color)' }}>{divisionName} Division</h2>
