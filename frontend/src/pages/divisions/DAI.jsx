...
const DAI = () => {
  ...
  useEffect(() => {
    const fetchDAIEmployees = async () => {
      try {
        ...
        const dai = divisionRes.data.find(d => d.name === 'DAI');
        if (!dai) {
          setError('DAI division not found');
          return;
        }

        const employeeRes = await axios.get('/api/employees', {
          headers: { Authorization: `Bearer ${token}` },
          params: { ids: dai.employeeIds.join(',') }
        });
        setEmployees(employeeRes.data);
      } catch (err) {
        setError('Failed to fetch employees');
        console.error('Error fetching DAI employees:', err);
      }
    };
    fetchDAIEmployees();
  }, [navigate]);
  ...
  const handleSaveEmployee = async () => {
    try {
      ...
      if (editMode) {
        ...
        const dai = divisionRes.data.find(d => d.name === 'DAI');
        if (dai) {
          await axios.put(`/api/divisions/${dai._id}`, {
            ...dai,
            employeeIds: [...dai.employeeIds, response.data._id]
          }, {
            headers: { Authorization: `Bearer ${token}` },
          });
        }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (dai) {
        await axios.put(`/api/divisions/${dai._id}`, {
          ...dai,
          employeeIds: dai.employeeIds.filter(id => id !== employeeToDelete._id)
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Employés DAI');
    XLSX.writeFile(wb, 'employes_dai.xlsx');
  };
  ...
  return (
    <div className="bg-white rounded-3 shadow-sm p-4 mx-auto" style={{ maxWidth: '1200px', margin: '20px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="mb-0 fw-semibold">Gestion des missions - DAI</h5>
