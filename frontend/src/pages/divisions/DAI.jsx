...
const DAIPage = ({ theme }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const dai = divisionResponse.data.find((div) => div.name === 'DAI');
      if (!dai) {
      ...
      const filteredEmployees = employeeResponse.data.filter((emp) => {
        ...
        return divisionId === dai._id || divisionId?.toString() === dai._id.toString();
      });
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DAI Employees');
      XLSX.writeFile(wb, 'dai_employees.xlsx');
    } catch (err) {
      ...
      <div className="container-lg">
        <div className="bg-white rounded-3 p-4 shadow-sm" style={{ backgroundColor: 'var(--card-bg)' }}>
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h2 className="mb-0" style={{ color: 'var(--text-color)' }}>DAI Division</h2>
