...
const DCT = () => {
  ...
  useEffect(() => {
    const fetchDCTEmployees = async () => {
      try {
        ...
        const dct = divisionRes.data.find(d => d.name === 'DCT');
        if (!dct) {
          setError('DCT division not found');
          return;
        }

        const employeeRes = await axios.get('/api/employees', {
          headers: { Authorization: `Bearer ${token}` },
          params: { ids: dct.employeeIds.join(',') }
        });
        setEmployees(employeeRes.data);
      } catch (err) {
        setError('Failed to fetch employees');
        console.error('Error fetching DCT employees:', err);
      }
    };
    fetchDCTEmployees();
  }, [navigate]);
  ...
  const handleSaveEmployee = async () => {
    try {
      ...
      if (editMode) {
        ...
        const dct = divisionRes.data.find(d => d.name === 'DCT');
        if (dct) {
          await axios.put(`/api/divisions/${dct._id}`, {
            ...dct,
            employeeIds: [...dct.employeeIds, response.data._id]
          }, {
            headers: { Authorization: `Bearer ${token}` },
          });
        }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (dct) {
        await axios.put(`/api/divisions/${dct._id}`, {
          ...dct,
          employeeIds: dct.employeeIds.filter(id => id !== employeeToDelete._id)
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Employés DCT');
    XLSX.writeFile(wb, 'employes_dct.xlsx');
  };
  ...
  return (
    <div className="bg-white rounded-3 shadow-sm p-4 mx-auto" style={{ maxWidth: '1200px', margin: '20px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="mb-0 fw-semibold">Gestion des missions - DCT</h5>
