...
const DCTPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const daec = divisionResponse.data.find((div) => div.name === 'DCT');
      if (!daec) {
      ...
      if (filteredEmployees.length === 0) {
        setError('No employees found for DCT');
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DCT Employees');
      XLSX.writeFile(wb, 'dct_employees.xlsx');
    } catch (err) {
