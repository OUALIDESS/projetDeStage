...
const DUEPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const daec = divisionResponse.data.find((div) => div.name === 'DUE');
      if (!daec) {
      ...
      if (filteredEmployees.length === 0) {
        setError('No employees found for DUE');
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DUE Employees');
      XLSX.writeFile(wb, 'due_employees.xlsx');
    } catch (err) {
