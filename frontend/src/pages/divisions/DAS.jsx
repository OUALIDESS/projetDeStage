...
const DAS = () => {
  ...
  useEffect(() => {
    const fetchDASEmployees = async () => {
      try {
        ...
        const das = divisionRes.data.find(d => d.name === 'DAS');
        if (!das) {
          setError('DAS division not found');
          return;
        }

        const employeeRes = await axios.get('/api/employees', {
          headers: { Authorization: `Bearer ${token}` },
          params: { ids: das.employeeIds.join(',') }
        });
        setEmployees(employeeRes.data);
      } catch (err) {
        setError('Failed to fetch employees');
        console.error('Error fetching DAS employees:', err);
      }
    };
    fetchDASEmployees();
  }, [navigate]);
  ...
  const handleSaveEmployee = async () => {
    try {
      ...
      if (editMode) {
        ...
        const das = divisionRes.data.find(d => d.name === 'DAS');
        if (das) {
          await axios.put(`/api/divisions/${das._id}`, {
            ...das,
            employeeIds: [...das.employeeIds, response.data._id]
          }, {
            headers: { Authorization: `Bearer ${token}` },
          });
        }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (das) {
        await axios.put(`/api/divisions/${das._id}`, {
          ...das,
          employeeIds: das.employeeIds.filter(id => id !== employeeToDelete._id)
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Employés DAS');
    XLSX.writeFile(wb, 'employes_das.xlsx');
  };
  ...
  return (
    <div className="bg-white rounded-3 shadow-sm p-4 mx-auto" style={{ maxWidth: '1200px', margin: '20px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="mb-0 fw-semibold">Gestion des missions - DAS</h5>
