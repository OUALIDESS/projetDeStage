...
const DASPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const daec = divisionResponse.data.find((div) => div.name === 'DAS');
      if (!daec) {
      ...
      if (filteredEmployees.length === 0) {
        setError('No employees found for DAS');
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DAS Employees');
      XLSX.writeFile(wb, 'das_employees.xlsx');
    } catch (err) {
