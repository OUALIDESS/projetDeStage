...
const DPEPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const daec = divisionResponse.data.find((div) => div.name === 'DPE');
      if (!daec) {
      ...
      if (filteredEmployees.length === 0) {
        setError('No employees found for DPE');
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DPE Employees');
      XLSX.writeFile(wb, 'dpe_employees.xlsx');
    } catch (err) {
