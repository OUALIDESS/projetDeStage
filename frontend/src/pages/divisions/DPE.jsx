...
const DPE = () => {
  ...
  useEffect(() => {
    const fetchDPEEmployees = async () => {
      try {
        ...
        const dpe = divisionRes.data.find(d => d.name === 'DPE');
        if (!dpe) {
          setError('DPE division not found');
          return;
        }

        const employeeRes = await axios.get('/api/employees', {
          headers: { Authorization: `Bearer ${token}` },
          params: { ids: dpe.employeeIds.join(',') }
        });
        setEmployees(employeeRes.data);
      } catch (err) {
        setError('Failed to fetch employees');
        console.error('Error fetching DPE employees:', err);
      }
    };
    fetchDPEEmployees();
  }, [navigate]);
  ...
  const handleSaveEmployee = async () => {
    try {
      ...
      if (editMode) {
        ...
        const dpe = divisionRes.data.find(d => d.name === 'DPE');
        if (dpe) {
          await axios.put(`/api/divisions/${dpe._id}`, {
            ...dpe,
            employeeIds: [...dpe.employeeIds, response.data._id]
          }, {
            headers: { Authorization: `Bearer ${token}` },
          });
        }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (dpe) {
        await axios.put(`/api/divisions/${dpe._id}`, {
          ...dpe,
          employeeIds: dpe.employeeIds.filter(id => id !== employeeToDelete._id)
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Employés DPE');
    XLSX.writeFile(wb, 'employes_dpe.xlsx');
  };
  ...
  return (
    <div className="bg-white rounded-3 shadow-sm p-4 mx-auto" style={{ maxWidth: '1200px', margin: '20px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="mb-0 fw-semibold">Gestion des missions - DPE</h5>
