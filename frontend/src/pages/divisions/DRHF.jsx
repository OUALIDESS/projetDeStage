...
const DRHF = () => {
  ...
  useEffect(() => {
    const fetchDRHFEmployees = async () => {
      try {
        ...
        const drhf = divisionRes.data.find(d => d.name === 'DRHF');
        if (!drhf) {
          setError('DRHF division not found');
          return;
        }

        const employeeRes = await axios.get('/api/employees', {
          headers: { Authorization: `Bearer ${token}` },
          params: { ids: drhf.employeeIds.join(',') }
        });
        setEmployees(employeeRes.data);
      } catch (err) {
        setError('Failed to fetch employees');
        console.error('Error fetching DRHF employees:', err);
      }
    };
    fetchDRHFEmployees();
  }, [navigate]);
  ...
  const handleSaveEmployee = async () => {
    try {
      ...
      if (editMode) {
        ...
        const drhf = divisionRes.data.find(d => d.name === 'DRHF');
        if (drhf) {
          await axios.put(`/api/divisions/${drhf._id}`, {
            ...drhf,
            employeeIds: [...drhf.employeeIds, response.data._id]
          }, {
            headers: { Authorization: `Bearer ${token}` },
          });
        }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (drhf) {
        await axios.put(`/api/divisions/${drhf._id}`, {
          ...drhf,
          employeeIds: drhf.employeeIds.filter(id => id !== employeeToDelete._id)
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Employés DRHF');
    XLSX.writeFile(wb, 'employes_drhf.xlsx');
  };
  ...
  return (
    <div className="bg-white rounded-3 shadow-sm p-4 mx-auto" style={{ maxWidth: '1200px', margin: '20px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="mb-0 fw-semibold">Gestion des missions - DRHF</h5>
