...
const DRHFPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const daec = divisionResponse.data.find((div) => div.name === 'DRHF');
      if (!daec) {
      ...
      if (filteredEmployees.length === 0) {
        setError('No employees found for DRHF');
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DRHF Employees');
      XLSX.writeFile(wb, 'drhf_employees.xlsx');
    } catch (err) {
