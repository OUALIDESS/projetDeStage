...
const DFL = () => {
  ...
  useEffect(() => {
    const fetchDFLEmployees = async () => {
      try {
        ...
        const dfl = divisionRes.data.find(d => d.name === 'DFL');
        if (!dfl) {
        ...
        const employeeRes = await axios.get('/api/employees', {
          headers: { Authorization: `Bearer ${token}` },
          params: { ids: dfl.employeeIds.join(',') }
        });
        setEmployees(employeeRes.data);
      } catch (err) {
        setError('Failed to fetch employees');
        console.error('Error fetching DFL employees:', err);
      }
    };
    fetchDFLEmployees();
  }, [navigate]);
  ...
  const handleSaveEmployee = async () => {
    try {
      ...
      if (editMode) {
        ...
        const dfl = divisionRes.data.find(d => d.name === 'DFL');
        if (dfl) {
          await axios.put(`/api/divisions/${dfl._id}`, {
            ...dfl,
            employeeIds: [...dfl.employeeIds, response.data._id]
          }, {
            headers: { Authorization: `Bearer ${token}` },
          });
        }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (dfl) {
        await axios.put(`/api/divisions/${dfl._id}`, {
          ...dfl,
          employeeIds: dfl.employeeIds.filter(id => id !== employeeToDelete._id)
        }, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Employés DFL');
    XLSX.writeFile(wb, 'employes_dfl.xlsx');
  };
  ...
  return (
    <div className="bg-white rounded-3 shadow-sm p-4 mx-auto" style={{ maxWidth: '1200px', margin: '20px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="mb-0 fw-semibold">Gestion des missions - DFL</h5>
