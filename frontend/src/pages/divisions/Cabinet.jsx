...
const Cabinet = ({ theme }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const daec = divisionResponse.data.find((div) => div.name === 'cabinet');
      if (!daec) {
      ...
      if (filteredEmployees.length === 0) {
        setError('No employees found for cabinet');
      }
    } catch (err) {
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Cabinet Employees');
      XLSX.writeFile(wb, 'cabinet_employees.xlsx');
    } catch (err) {
